...
/**
 * Verifie l'application effective de la lecture seule stricte au runtime (adapte de
 * tests/Database/ConnectionTest.php du site francais) : lecture possible, ecriture
 * impossible par construction, aucun fichier cree en cas de chemin absent.
 */
return function (): void {
    ...
    $row = $pdo->query("SELECT normalized FROM terms WHERE normalized = 'COCHE' LIMIT 1")->fetch();
    Assert::notNull($row, 'lecture attendue sur une connexion en lecture seule');
    Assert::same('COCHE', $row['normalized']);
    ...
    // is_spanish (pas is_french, voir schema.sql) : seul le nom de colonne change, meme
    // discipline is_ods8/is_ods9 conservee (voir schema.sql pour la raison).
    $wrote = true;
    try {
        $pdo->exec(
            "INSERT INTO terms (display_term, normalized, is_spanish, is_ods8, is_ods9, "
